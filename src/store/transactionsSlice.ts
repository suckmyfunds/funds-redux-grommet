import { createAction, createAsyncThunk, createEntityAdapter, createReducer, createSelector, createSlice } from "@reduxjs/toolkit";
...
export const fetchTransactionsForFund = createAsyncThunk("transactions/fetchAll",
    async (fundName: string, { getState }): Promise<TransactionRemote[]> => {
        ...
        return (await api.getRows(`${fundName}!A2:E`)).map((t, idx) => (
            {
                ...transformTransactionFromResponse(t), id: String(idx + 2), status: 'idle', fundId,
            }));
    });
...
export const selectTransactionsByFundId = createSelector(
    [selectAllTransactions],
    (trs) => {
